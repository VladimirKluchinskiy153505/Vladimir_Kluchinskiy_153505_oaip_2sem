...
size_t n;
size_t Count = 1;
void GetC(my_vec<MyString>& A, my_vec<MyString>& B, my_vec<MyString>& C) {
...
bool div2(MyString& str, int& pos) {
    int x = str.size(), a = 0;
    int* arr = new int[x];
...
void rec(my_vec<MyString>& A, my_vec<MyString>& B, my_vec<MyString>& C) {
    int k = 1;
    while (true) {
        size_t Csize = C.size();
